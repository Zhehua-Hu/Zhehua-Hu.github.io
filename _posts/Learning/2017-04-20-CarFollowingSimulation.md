...
def round_float(num, digit=1):
    return float(("%." + str(int(digit)) + "f") % num)
...
class FirstCar:
    ...
    def update(self):
        ...
        print("--->[1]<---", round_float(self._abs_acc), round_float(self._abs_speed), round_float(self._abs_pos))
...
class Car:
    ...
    def update(self, pre_car):
        ...
        if self._use_nonlinear:
            ...
            # self._abs_acc = safe_acc((self._C/self._tao) * (pre_car._abs_speed - self._abs_speed) / ((pre_car._abs_pos - self._abs_pos)*0.001)**3)
        else:
        ...
        # print(">>>", round_float(self._rel_dis_pre),
        #             round_float(self._abs_pos),
        #             round_float(self._abs_speed),
        #             round_float(self._abs_acc))
        self._abs_acc_list.append(self._abs_acc)
        self._abs_speed_list.append(self._abs_speed)
        self._abs_pos_list.append(self._abs_pos)
...
while True:
    # 设置变速条件
    # if m_start_dec < first_car.get_pos() < m_start_acc:
    #     first_car.set_acc(norm_dec)
    #     if first_car.get_speed() < min_speed:
    #         first_car.set_acc(0.0)
    # if m_start_acc < first_car.get_pos() < m_stop_line:
    ...
    if (m_start_dec < first_car.get_pos() < m_start_acc) and (flag1!=True):
        first_car.set_speed(min_speed)
        flag1 = True
    if (m_start_acc < first_car.get_pos() < m_stop_line) and (flag2!=True):
        first_car.set_speed(speed_init)
        flag2 = True
    if cars[-1].get_pos() > m_stop_line:
        break
...
for i in range(len(t)):
    ...
    for item in database:
        row.append(round_float(item[0][i]))
        row.append(round_float(item[1][i]))
        row.append(round_float(item[2][i]))
    data.append(row)
